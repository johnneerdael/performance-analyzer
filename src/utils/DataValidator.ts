...
export class DataValidator {
  ...
  validateDataset(dataset: Dataset): ValidationResult<Dataset> {
    ...
    if (!isValid) {
      this.errorHandler.handleValidationError(
        new Error(`Dataset validation failed with ${errors.length} errors`),
        'Dataset',
  ...
  validateTestParameters(params: TestParameters): ValidationResult<TestParameters> {
    ...
    if (!isValid) {
      this.errorHandler.handleValidationError(
        new Error(`Test parameters validation failed with ${errors.length} errors`),
        'TestParameters',
        'unknown'
      );
    }
  ...
  validateTestResults(results: TestResults): ValidationResult<TestResults> {
    ...
    if (!isValid) {
      this.errorHandler.handleValidationError(
        new Error(`Test results validation failed with ${errors.length} errors`),
        'TestResults',
        'unknown'
      );
    }
   ...
   */
  private validateIperfTestResult(test: IperfTestResult): ValidationError[] {
    const errors: ValidationError[] = [];
   ...
   * @returns Array of validation errors
   */
  private validateDnsTestResult(test: DnsTestResult): ValidationError[] {
    const errors: ValidationError[] = [];
   ...
   */
  private sanitizeTestResults(results: TestResults): TestResults {
    const sanitized: TestResults = {
      iperfTests: [],
      dnsResults: []
    };
   ...
   */
  private sanitizeIperfTestResult(test: IperfTestResult): IperfTestResult {
    const sanitized: IperfTestResult = {
   ...
   */
  private sanitizeDnsTestResult(test: DnsTestResult): DnsTestResult {
    const sanitized: DnsTestResult = {
