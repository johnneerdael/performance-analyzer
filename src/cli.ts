...
async function validateInputDirectory(inputDir: string): Promise<boolean> {
  try {
    ...
    const contents = await fs.readdir(inputDir, { withFileTypes: true });
    
    // Check for flat structure (parameter/result files directly in directory)
    const parameterFiles = contents.filter(item => 
      item.isFile() && item.name.startsWith('parameters-results_')
    );
    const resultFiles = contents.filter(item => 
      item.isFile() && item.name.startsWith('results_')
    );
    
    // Check for subdirectory structure (legacy)
    const potentialDatasetDirs = contents.filter(item => 
      item.isDirectory() && (item.name.startsWith('coredns-mtu') || item.name.startsWith('stock-mtu'))
    );
