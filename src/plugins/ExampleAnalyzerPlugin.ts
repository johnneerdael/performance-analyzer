 ...
 */
class ExampleAnalyzerPlugin implements Plugin {
  name = 'example-analyzer';
  description = 'Example plugin that calculates additional performance metrics';
  version = '1.0.0';
  private config: any = {};
   ...
   * @private
   */
  private calculateBandwidthToLatencyRatio(datasets: any[]): any[] {
    const results: any[] = [];
   ...
   * @private
   */
  private calculateConfigurationStability(datasets: any[]): any[] {
    const results: any[] = [];
    ...
    for (const [config, configDatasets] of Object.entries(configGroups)) {
      ...
      for (const dataset of configDatasets) {
        if (dataset.results && dataset.results.iperfTests) {
          for (const test of dataset.results.iperfTests) {
            if (test.bandwidthMbps) bandwidthValues.push(test.bandwidthMbps);
            if (test.jitterMs) latencyValues.push(test.jitterMs);
            if (test.packetLoss) packetLossValues.push(test.packetLoss);
          }
        }
      }
      ...
      const overallStability = (
        bandwidthStability * (this.config.bandwidthWeight || 0.4) +
   ...
   * @private
   */
  private calculatePerformanceIndex(datasets: any[]): any[] {
    const results: any[] = [];
