...
export class PluginManager {
  ...
  registerPlugin(plugin: Plugin, options: PluginRegistrationOptions = {}): PluginManager {
    ...
    if (!this.pluginMetadata.has(plugin.name)) {
      ...
      metadata.enabled = options.enabled !== undefined ? options.enabled : metadata.enabled;
    }
  ...
  enablePlugin(pluginName: string): PluginManager {
    ...
    if (metadata) {
      ...
      if (pluginConfig && pluginConfig.enabled && !pluginConfig.enabled.includes(pluginName)) {
