// DnsAnalyzer Service Implementation
import {
  Dataset,
  DnsTestResult,
...
export class DnsAnalyzer {
  ...
  calculatePerformanceMetrics(datasets: Dataset[]): DnsPerformanceMetrics[] {
    try {
      return datasets.map(dataset => {
        ...
        const sortedDomains = [...domainPerformance].sort((a, b) => 
  ...
  calculateDomainRankings(datasets: Dataset[]): DomainPerformance[] {
    try {
      ...
      const domainPerformance: DomainPerformance[] = Array.from(allDomains).map(domain => {
  ...
  calculateServerComparison(datasets: Dataset[]): DnsServerComparison[] {
    try {
      ...
      const serverComparison: DnsServerComparison[] = Array.from(allServers).map(server => {
        let totalResponseTime = 0;
        let successCount = 0;
        let totalCount = 0;
   ...
   */
  private calculateDomainPerformanceForDataset(dataset: Dataset): DomainPerformance[] {
    try {
      const dnsResults = this.getDnsResults(dataset);
      ...
      return Array.from(domains).map(domain => {
        ...
        const successfulResults = domainResults.filter(result => 
          result.success && result.responseTimeMs !== undefined
        );
        
        const responseTimes = successfulResults.map(result => result.responseTimeMs as number);
   ...
   */
  private getDnsResults(dataset: Dataset): DnsTestResult[] {
    try {
      ...
      return (dataset as any).results?.dnsResults || [];
    } catch (error) {
      const analysisError = new Error(error instanceof Error ? error.message : String(error)) as AnalysisError;
