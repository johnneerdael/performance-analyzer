...
export class DefaultReportGenerator implements ReportGenerator {
   ...
   */
  private generateAdvancedAnalysisTables(analysis: AnalysisResults): string {
   ...
   */
  private generateDetailedBandwidthAnalysis(metrics: BandwidthMetrics[]): string {
    ...
    sortedByCv.forEach(metric => {
      ...
      table.push(
        `| ${metric.displayName} | ${metric.avgBandwidthMbps.toFixed(2)} | ${metric.medianBandwidthMbps.toFixed(2)} | ${metric.standardDeviation.toFixed(2)} | ${(metric.cv * 100).toFixed(2)} | ${metric.minBandwidthMbps.toFixed(2)} | ${percentile25.toFixed(2)} | ${percentile75.toFixed(2)} | ${metric.percentile95.toFixed(2)} | ${metric.percentile99.toFixed(2)} | ${metric.maxBandwidthMbps.toFixed(2)} |`
      );
    });
   ...
   */
  private generateJitterAnalysis(metrics: LatencyMetrics[]): string {
    ...
    sortedByJitter.forEach(metric => {
      ...
      table.push(
        `| ${metric.displayName} | ${metric.jitterMs.toFixed(2)} | ${metric.avgLatencyMs.toFixed(2)} | ${metric.jitterLatencyRatio.toFixed(4)} | ${metric.maxLatencyMs.toFixed(2)} | ${metric.minLatencyMs.toFixed(2)} | ${latencyRange.toFixed(2)} |`
      );
    });
   ...
   */
  private generateRetransmissionAnalysis(metrics: ReliabilityMetrics[]): string {
    ...
    sortedByRetransmit.forEach(metric => {
      ...
      table.push(
        `| ${metric.displayName} | ${(metric.retransmitRate * 100).toFixed(2)} | ${(metric.packetLossRate * 100).toFixed(2)} | ${(metric.successRate * 100).toFixed(2)} | ${metric.errorCount} | ${retransmitLossRatio} |`
      );
    });
   ...
   */
  private generatePerformanceCorrelationMatrix(analysis: AnalysisResults): string {
      ...
      metrics.forEach(otherMetric => {
        if (metric.key === otherMetric.key) {
          ...
          const sortFunc = metric.higherIsBetter ? 
            (a: any, b: any) => b[metric.key] - a[metric.key] : 
