...
`;
    ...
    async discoverDatasets(rootPath) {
        try {
            ...
            datasets.forEach((dataset) => {
                this.logProgress(`Found dataset: ${dataset.name} (MTU: ${dataset.configuration.mtu}, DNS Query Logging: ${dataset.configuration.awsLogging ? "enabled" : "disabled"})`);
            });
            return datasets;
        }
        catch (error) {
    ...
    createExecutiveSummary(datasets, iperfAnalysis, dnsAnalysis, configurationComparison, anomalies) {
        ...
        if (configurationComparison.loggingImpact) {
            ...
            if (magnitude > 0.05) {
                ...
                keyFindings.push(`DNS query logging ${direction} overall performance by approximately ${(magnitude * 100).toFixed(1)}%.`);
            }
            else {
                keyFindings.push("DNS query logging has minimal impact on overall network performance.");
            }
        }
     ...
     */
    prepareLoggingImpactData(analysis) {
        const { iperfAnalysis, dnsAnalysis, configurationComparison } = analysis;
        const { bandwidthComparison, latencyAnalysis, reliabilityMetrics } = iperfAnalysis;
        const { performanceMetrics } = dnsAnalysis;
        const { overallRanking } = configurationComparison;
     ...
     */
    prepareAnomalyDistributionData(anomalies) {
        ...
        async generateReport(analysis) {
            try {
                ...
                if (this.datasetDisplayNames.size > 0) {
                    console.log('[DEBUG] Using dataset display names:', Array.from(this.datasetDisplayNames.entries())
                ...
                if (this.datasetDisplayNames.size > 0) {
                    ...
                    if (reportData.performanceHighlights && Array.isArray(reportData.performanceHighlights)) {
                        reportData.performanceHighlights = reportData.performanceHighlights.map((highlight) => {
                    ...
                    if (reportData.configurations && Array.isArray(reportData.configurations)) {
                        reportData.configurations = reportData.configurations.map((config) => ({
                            ...config,
                    ...
                    if (reportData.anomalies && Array.isArray(reportData.anomalies)) {
                        reportData.anomalies = reportData.anomalies.map((anomaly) => ({
                            ...anomaly,
