...
class AnomalyDetector {
     ...
     */
    detectBandwidthAnomalies(datasets) {
        const anomalies = [];
        // Extract all bandwidth metrics from iperf tests
        const allBandwidthValues = [];
        const bandwidthByConfig = {};
        ...
        if (allBandwidthValues.length === 0)
            return anomalies;
        ...
        Object.entries(bandwidthByConfig).forEach(([config, values]) => {
            ...
            if (configAvg < this.config.bandwidthMinimumThreshold) {
                const severity = this.determineSeverity(this.config.bandwidthMinimumThreshold / configAvg);
                anomalies.push({
                    type: 'bandwidth',
            ...
            if (deviation > this.config.bandwidthDeviationThreshold) {
                const severity = this.determineSeverity(deviation / this.config.bandwidthDeviationThreshold);
    ...
    detectLatencyAnomalies(datasets) {
        ...
        Object.entries(jitterByConfig).forEach(([config, values]) => {
            ...
            if (configAvg > this.config.latencyMaximumThreshold) {
                const severity = this.determineSeverity(configAvg / this.config.latencyMaximumThreshold);
                anomalies.push({
                    type: 'latency',
                    configuration: config,
            ...
            if (deviation > this.config.latencyDeviationThreshold) {
                const severity = this.determineSeverity(deviation / this.config.latencyDeviationThreshold);
     ...
     */
    detectPacketLossAnomalies(datasets) {
        const anomalies = [];
        // Process UDP packet loss
        const packetLossByConfig = {};
        // Process TCP retransmits
        const retransmitsByConfig = {};
        // Process each dataset
        datasets.forEach(dataset => {
            if (!dataset.results?.iperfTests)
                return;
        ...
        Object.entries(packetLossByConfig).forEach(([config, values]) => {
            ...
            if (avgLoss > this.config.packetLossThreshold) {
                const severity = this.determineSeverity(avgLoss / this.config.packetLossThreshold);
                anomalies.push({
                    type: 'packet_loss',
                    configuration: config,
        ...
        Object.entries(retransmitsByConfig).forEach(([config, values]) => {
            ...
            if (avgRetransmitRate > this.config.retransmitRateThreshold) {
                const severity = this.determineSeverity(avgRetransmitRate / this.config.retransmitRateThreshold);
                anomalies.push({
                    type: 'packet_loss',
                    configuration: config,
     ...
     */
    detectDnsAnomalies(datasets) {
        const anomalies = [];
        // Extract DNS response times by domain and configuration
        const responseTimesByDomain = {};
        const responseTimesByConfig = {};
        const successRateByConfig = {};
        // Process each dataset
        datasets.forEach(dataset => {
        ...
        Object.entries(responseTimesByConfig).forEach(([config, times]) => {
            ...
            if (avgResponseTime > this.config.dnsResponseTimeThreshold) {
                const severity = this.determineSeverity(avgResponseTime / this.config.dnsResponseTimeThreshold);
                anomalies.push({
                    type: 'dns_failure',
                    configuration: config,
        ...
        Object.entries(successRateByConfig).forEach(([config, { success, total }]) => {
            ...
            if (successRate < this.config.dnsSuccessRateThreshold) {
                const severity = this.determineSeverity((1 - successRate) / (1 - this.config.dnsSuccessRateThreshold));
                anomalies.push({
                    type: 'dns_failure',
                    configuration: config,
        ...
        Object.entries(responseTimesByDomain).forEach(([domain, times]) => {
            if (times.length < 2)
                return; // Need at least a couple of samples for test cases
            const avgResponseTime = this.calculateMean(times);
            if (avgResponseTime > this.config.dnsResponseTimeThreshold * 1.5) {
                const severity = this.determineSeverity(avgResponseTime / (this.config.dnsResponseTimeThreshold * 1.5));
