...
exports.ConfigurationComparator = void 0;
...
class ConfigurationComparator {
     ...
     */
    compareConfigurations(datasets) {
        try {
            // Analyze MTU impact
    ...
    analyzeLoggingImpact(datasets) {
        try {
            ...
            const recommendations = this.generateLoggingRecommendations(performanceImpact, bandwidthDifference, latencyDifference);
            return {
                performanceImpact,
                bandwidthDifference,
                latencyDifference,
    ...
    rankConfigurations(datasets) {
        try {
            ...
            for (const dataset of datasets) {
                ...
                const overallScore = ((bandwidthScore * 0.5) + // 50% weight to bandwidth
     ...
     */
    analyzePerformanceTrends(datasets) {
        try {
            // Group datasets by MTU
            const datasetsByMtu = this.groupDatasetsByMtu(datasets);
     ...
     */
    groupDatasetsByServer(datasets) {
        const groups = {};
        for (const dataset of datasets) {
     ...
     */
    groupDatasetsByLogging(datasets) {
        const groups = {};
        for (const dataset of datasets) {
     ...
     */
    analyzeTrendsByGroup(groupedDatasets, groupType) {
        const trends = {};
        for (const [groupKey, datasets] of Object.entries(groupedDatasets)) {
            ...
            const avgDnsResponseTime = this.calculateAverageMetric(dnsAnalysis.performanceMetrics, 'avgResponseTimeMs');
            trends[groupKey] = {
                avgBandwidth,
                avgLatency,
                successRate,
     ...
     */
    calculateAverageMetric(metrics, metricKey) {
        if (metrics.length === 0)
            return 0;
        let sum = 0;
        let count = 0;
     ...
     */
    generateMtuRecommendations(performanceByMtu, optimalMtu) {
        ...
        if (performanceByMtu[1500] && optimalMtu !== 1500) {
            ...
            if (optimalPerf && standardPerf) {
                ...
                if (bandwidthPercent >= 5) {
                    recommendations.push(`Using MTU ${optimalMtu} provides a ${bandwidthPercent.toFixed(1)}% bandwidth improvement over standard MTU 1500.`);
                }
            }
        }
        ...
        if (jumboMtus.length > 0) {
            ...
            if (performanceByMtu[1500] && jumboPerf) {
                ...
                if (bandwidthPercent >= 10) {
                    recommendations.push(`Jumbo frames (MTU ${bestJumboMtu}) provide significant bandwidth benefits (${bandwidthPercent.toFixed(1)}% improvement).`);
                }
                else if (bandwidthPercent <= -5) {
                    recommendations.push(`Jumbo frames (MTU ${bestJumboMtu}) show worse performance than standard MTU 1500. Check network path for MTU black holes.`);
                }
            }
        }
        ...
        if (smallMtus.length > 0) {
            ...
            if (performanceByMtu[1500] && smallPerf) {
                ...
                if (latencyPercent >= 10) {
                    recommendations.push(`Smaller MTU (${smallestMtu}) shows lower latency (${latencyPercent.toFixed(1)}% improvement) which may benefit real-time applications.`);
                }
            }
        }
     ...
     * @returns Array of recommendation strings
     */
    generateLoggingRecommendations(performanceImpact, bandwidthDifference, latencyDifference) {
        const recommendations = [];
...
exports.ConfigurationComparator = ConfigurationComparator;
//# sourceMappingURL=ConfigurationComparator.js.map