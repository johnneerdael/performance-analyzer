...
class TemplateParser {
     ...
     * @private
     */
    calculateLocation(template, start, end) {
        let line = 1;
        let column = 1;
     ...
     * @private
     */
    parseTokens(tokens, template) {
        const root = (0, TemplateAST_1.createRootNode)();
        const stack = [root];
        for (let i = 0; i < tokens.length; i++) {
            ...
            if (token.type === 'expression' && token.isElse) {
                const current = stack[stack.length - 1];
                if (!current)
                    continue;
                if (current.type === TemplateAST_1.TemplateNodeType.IF) {
                    ...
                    stack.pop();
                    const elseIfNode = {
                        type: TemplateAST_1.TemplateNodeType.IF,
                        condition: current.condition,
                        original: current.original,
                        location: current.location,
                        children: current.else
                    };
            ...
            switch (token.type) {
                case 'text':
                    if (current.type === TemplateAST_1.TemplateNodeType.ROOT ||
                        current.type === TemplateAST_1.TemplateNodeType.EACH ||
                        current.type === TemplateAST_1.TemplateNodeType.IF ||
                        current.type === TemplateAST_1.TemplateNodeType.HELPER) {
                        const textNode = (0, TemplateAST_1.createTextNode)(token.value, {
                            start: token.start,
                            end: token.end,
                            line: token.line,
                            column: token.column
                ...
                case 'expression':
                    if (current.type === TemplateAST_1.TemplateNodeType.ROOT ||
                        ...
                        if (helperMatch && helperMatch[1] && helperMatch[2]) {
                            ...
                            const helperNode = (0, TemplateAST_1.createHelperNode)(helperName, args, `{{${token.value}}}`, undefined, {
                                start: token.start,
                                end: token.end,
                                line: token.line,
                                column: token.column
                            });
                            current.children.push(helperNode);
                        }
                        else {
                            // It's a variable
                            const variableNode = (0, TemplateAST_1.createVariableNode)(token.value, `{{${token.value}}}`, {
                                start: token.start,
                                end: token.end,
                                line: token.line,
                                column: token.column
                ...
                case 'block-start':
                    if (current.type === TemplateAST_1.TemplateNodeType.ROOT ||
                        ...
                        if (blockType === 'each') {
                            const eachNode = (0, TemplateAST_1.createEachNode)(expression, [], `{{#each ${expression}}}`, token.itemName, token.indexName, {
                                start: token.start,
                                end: token.end,
                                line: token.line,
                                column: token.column
                            ...
                            const ifNode = (0, TemplateAST_1.createIfNode)(expression, [], `{{#if ${expression}}}`, undefined, {
                                start: token.start,
                                end: token.end,
                                line: token.line,
                                column: token.column
                            ...
                            // Custom block helper
                            const helperNode = (0, TemplateAST_1.createHelperNode)(blockType, [expression], `{{#${blockType} ${expression}}}`, [], {
                                start: token.start,
                                end: token.end,
                                line: token.line,
                                column: token.column
                ...
                case 'block-end':
                    const blockType = token.blockType;
                    if (blockType === 'each' && current.type === TemplateAST_1.TemplateNodeType.EACH) {
                        stack.pop();
                    }
                    else if (blockType === 'if' && current.type === TemplateAST_1.TemplateNodeType.IF) {
                        stack.pop();
                    }
                    else if (current.type === TemplateAST_1.TemplateNodeType.HELPER && current.name === blockType) {
                        stack.pop();
                    }
                    else {
                ...
                case 'comment':
                    if (current.type === TemplateAST_1.TemplateNodeType.ROOT ||
                        ...
                        current.type === TemplateAST_1.TemplateNodeType.IF ||
                        current.type === TemplateAST_1.TemplateNodeType.HELPER) {
                        const commentNode = (0, TemplateAST_1.createCommentNode)(token.value, `{{!-- ${token.value} --}}`, {
                            start: token.start,
                            end: token.end,
                            line: token.line,
                            column: token.column
        ...
        if (stack.length > 1) {
            ...
            let blockType = '';
            if (unclosed.type === TemplateAST_1.TemplateNodeType.EACH) {
                blockType = 'each';
            }
            else if (unclosed.type === TemplateAST_1.TemplateNodeType.IF) {
                blockType = 'if';
            }
            else if (unclosed.type === TemplateAST_1.TemplateNodeType.HELPER) {
                blockType = unclosed.name;
            }
            throw new Error(`Unclosed block: {{#${blockType}}} at line ${(unclosed.location?.line || 0)}, column ${(unclosed.location?.column || 0)}`);
        }
        return (0, TemplateAST_1.createTemplateAST)(root.children, template);
    }
    /**
     ...
     * @private
     */
    validateTagBalance(tokens, errors) {
        const stack = [];
        for (const token of tokens) {
            if (token.type === 'block-start') {
                stack.push({ type: token.blockType, token });
            }
            else if (token.type === 'block-end') {
                if (stack.length === 0) {
                    errors.push({
                        type: 'syntax',
                        message: `Unexpected closing tag: {{/${token.blockType}}}`,
                        line: token.line,
                        column: token.column,
                        context: `...{{/${token.blockType}}}...`
                    });
                }
                else {
                    const last = stack.pop();
                    if (last && last.type !== token.blockType) {
                        errors.push({
                            type: 'syntax',
                            message: `Mismatched closing tag: expected {{/${last.type}}}, found {{/${token.blockType}}}`,
                            line: token.line,
                            column: token.column,
                            context: `...{{/${token.blockType}}}...`
                        });
     ...
     * @private
     */
    validateExpressions(tokens, errors, warnings) {
        for (const token of tokens) {
     ...
     * @private
     */
    validateBlockNesting(tokens, errors, warnings) {
        let nestingLevel = 0;
        let eachNestingLevel = 0;
        for (const token of tokens) {
            if (token.type === 'block-start') {
                nestingLevel++;
                if (token.blockType === 'each') {
                    eachNestingLevel++;
     ...
     * @private
     */
    checkPerformance(template, warnings) {
